/**
 * LoginPage — Authentication & Registration Page
 * =================================================
 * Dual-purpose page for signing in and creating new accounts.
 *
 * Features:
 *   - Toggle between Sign In / Create Account tabs
 *   - Role selection (Teacher/HOD) during registration
 *   - University & Department dropdowns for institutional metadata
 *   - Password visibility toggle, error display, loading spinner
 *
 * Props:
 *   @param {function} onLogin - Callback: (role, token, fullName) => void
 *   @param {function} onBack  - Navigate back to landing page
 */
...
// Role options for registration toggle
const ROLES = [
...
export default function LoginPage({ onLogin, onBack }) {
  ...
  useEffect(() => { setErrorMsg(null) }, [isSignUp])
    ...
    <div className="hero-section min-h-screen flex items-center justify-center p-4 relative">
      ...
      <div className="relative w-full max-w-md z-10 animate-fade-up">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-4 shadow-lg" style={{ background: 'linear-gradient(135deg, var(--brand-primary), var(--brand-dark))', boxShadow: 'var(--shadow-glow)' }}>
        ...
        <div className="glass rounded-[2rem] p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6">
                  ...
                  {ROLES.map(role => {
                    ...
                    return (
                      <button key={role.id} type="button" onClick={() => setSelectedRole(role.id)}
                              ...
                              style={{ background: isSelected ? (role.type === 'brand' ? 'var(--brand-bg)' : 'var(--accent-bg)') : 'var(--bg-input)', borderColor: isSelected ? colorBase : 'transparent', color: isSelected ? colorBase : 'var(--text-muted)' }}>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} placeholder="Password" required value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full pl-11 pr-12 py-3 rounded-xl border text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)] transition-all" style={{ background: 'var(--bg-input)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 focus:outline-none transition-colors hover:text-[var(--text-primary)]" style={{ color: 'var(--text-muted)' }} title={showPassword ? 'Hide password' : 'Show password'}>
            ...
            {/* Submit */}
            <button type="submit" disabled={isAuthenticating} className="w-full py-3.5 flex items-center justify-center gap-2 rounded-xl font-bold text-sm transition-all shadow-lg text-white mt-2 cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed" style={{ background: 'linear-gradient(90deg, var(--brand-dark), var(--brand-light))' }}>
